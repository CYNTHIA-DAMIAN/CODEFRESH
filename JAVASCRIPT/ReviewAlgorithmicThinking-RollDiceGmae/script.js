...
const checkForStraights = (arr) => {
  ...
  const sequence = uniqueSorted.join(""); // Convert to string

  const smallStraights = ["1234", "2345", "3456"];
  const largeStraights = ["12345", "23456"];

  let isLargeStraight = largeStraights.includes(sequence);
  let isSmallStraight = smallStraights.some(s => sequence.includes(s));
  ...
  if (!isLargeStraight && !isSmallStraight) {
...
/* Different ways of writting code for toggle  1 is by me and two is by freecodecamp 
rulesBtn.addEventListener("click", () => {
  isModalShowing = !isModalShowing;
